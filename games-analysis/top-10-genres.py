...
plt.ba(df_follow.index - BAR_WIDTH / 2, df_follow["total_followers"], color = "blue", label = "Number of Follows", width = BAR_WIDTH)

plt.bar(df_hype.index + BAR_WIDTH / 2, df_hype["total_hypes"], color = "red", label = "Number of hypes", width = BAR_WIDTH)

plt.xsticks(df_follow.index, df_follow["genre_name"])
